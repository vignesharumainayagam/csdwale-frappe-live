...
def get_context(path, args=None):
	...
	context.city = frappe.db.get_list("City", fields=['city_name', 'name', 'route'],filters={'is_published':1},order_by='city_name',limit_page_length=200)

	brands = frappe.db.get_list("ItemBrand", fields=['brand_name', 'name', 'route'],filters={'published':1}, limit_page_length=10, order_by='creation')

	for brand in brands:
		category_route=frappe.db.get_list('Item Brand Category',fields='category',filters={'parent':brand.name})
		brand.category_route=frappe.db.get_value('Category',fieldname=['route'], filters={'name': category_route[0].category})

	context.brands = brands

	context.item_categories = frappe.db.get_all("Category", fields=['category_name', 'route'],filters={'published':1}, order_by="name")
...
def build_context(context):
	...
	if not context.base_template_path:
		...
		context.web_base_template_path = app_base[0] if app_base else "templates/webbase.html"
...
def get_cities():
	city_name = frappe.db.get_list("City", fields=['city_name', 'name', 'route'],filters={'is_published':1},order_by='city_name',limit_page_length=200)
	return city_name
